...
class baiduSpeech(object):
    ...
    def fetch_token(self):
        ...
        try:
            req = requests.post(token_url, headers = {'Content-Type':'application/json; charset=UTF-8'}, data = body)
            ...
            if ('access_token' in result.keys() and 'scope' in result.keys()):
                ...
                with open(TOKEN_PATH, "w") as f:
                    result = { key:result.get(key) for key in ['access_token', 'expires_in']}
    ...
    def load(self):
        ...
        try:
            with open(TOKEN_PATH, 'r') as f:
                ...
                elif time.time() - access_token['expires_in'] - access_token['get_time'] >= 0:
                    return self.fetch_token()
                else:
